...
public class HttpEngine {
    ...
    //filled up within the constructor used for getting various info such as URLs, get defaultPort(), 
    //get chunked Length etc.
    protected HttpURLConnectionImpl policy;

    protected final String method;

    
    //enum with vals CACHE, CONDITIONAL_CACHE, NETWORK
    private ResponseSource responseSource;

    //filled up with OpenSocketConnection() or constructor,
    //getInput and OutputStreams 
    //HttpConnection.connect() method used to get a new connection object from connection pool 
    protected HttpConnection connection;
    
    //filled with connection.getInputStream 
    //used to initialize content stream (response body stream) that will be returned to the user to read from.
    private InputStream socketIn;
    
    //filled up with conenction.outStream
    //same as requestOut- write request to socket
    ...
    //used for writing request body.
    private AbstractHttpOutputStream requestBodyOut;

    //the response filled up from socketIn or from cache, will be returned in getResponseBody(), 
    //which is used by many functions like getInputStream in HttpURLConnectionImpl 
    ...
    public final void sendRequest() throws IOException {
    	System.out.println("MIC: HttpEngine: sendRequest()");
...
//	}
    ...
    protected final HttpConnection openSocketConnection() throws IOException {
    	...
    	result = HttpConnection.connect(uri, getSslSocketFactory(), policy.getProxy(), requiresTunnel(), policy.getConnectTimeout(), TYPE);
    ...
    public final void readResponse() throws IOException {
        ...
        if (sentRequestMillis == -1) {
            ...
            try{
            	...
            	//System.out.println("MIC: HttpEngine:: readResponse() :: contentLength");
            }
            catch(Exception e){
